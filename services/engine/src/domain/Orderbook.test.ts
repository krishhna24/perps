...
const order = (
  o: Partial<Order> & Pick<Order, "id" | "userId" | "side" | "entryPrice" | "quantity">,
): Order => ({
  leverage: 1,
  filled: "0",
  ...o,
...
describe("Orderbook matching", () => {
  test("a crossing bid fully fills against a resting ask at the ask price", () => {
    ...
    expect(res.executedQty.toFixed()).toBe("5");
  ...
  test("partial fill: remainder rests with filled set", () => {
    ...
    expect(res.executedQty.toFixed()).toBe("3");
  ...
  test("self-trade prevention: a user does not match their own resting order", () => {
    ...
    expect(res.executedQty.toFixed()).toBe("0");
  ...
  test("a partially-filled resting maker is never over-filled (uses remaining size)", () => {
    ...
    expect(res.executedQty.toFixed()).toBe("6");
  ...
  test("matching stops at the first non-crossing level rather than walking past it", () => {
    const book = new Orderbook(
      [],
      [
        order({ id: "a1", userId: "m1", side: "SHORT", entryPrice: "100", quantity: "1" }),
        order({ id: "a2", userId: "m2", side: "SHORT", entryPrice: "110", quantity: "5" }),
      ],
    );

    const res = book.addOrder(order({ id: "b1", userId: "tk", side: "LONG", entryPrice: "105", quantity: "6" }));

    expect(res.executedQty.toFixed()).toBe("1");
    expect(res.fills).toHaveLength(1);
    expect(book.asks).toHaveLength(1);
    expect(book.asks[0]!.entryPrice).toBe("110");
  });

  test("price-time priority: at one price the earlier order fills first", () => {
    const book = new Orderbook(
      [],
      [
        order({ id: "first", userId: "m1", side: "SHORT", entryPrice: "100", quantity: "5" }),
        order({ id: "second", userId: "m2", side: "SHORT", entryPrice: "100", quantity: "5" }),
      ],
    );

    const res = book.addOrder(order({ id: "b1", userId: "tk", side: "LONG", entryPrice: "100", quantity: "5" }));

    expect(res.fills).toHaveLength(1);
    expect(res.fills[0]!.otherOrderId).toBe("first");
  });
});
...
describe("Orderbook depth & helpers", () => {
  test("depth advertises only remaining (unfilled) size", () => {
    const book = new Orderbook(
      [],
      [order({ id: "a1", userId: "maker", side: "SHORT", entryPrice: "100", quantity: "10", filled: "4" })],
    );
  ...
  test("a market order (allowResting=false) never leaves a resting remainder", () => {
    ...
    expect(res.executedQty.toFixed()).toBe("4");
    expect(book.bids).toHaveLength(0);
    expect(book.asks).toHaveLength(0);
  });

  test("cancelOrder removes the resting order and returns it", () => {
    const book = new Orderbook([order({ id: "b1", userId: "u", side: "LONG", entryPrice: "99", quantity: "5" })], []);

    const removed = book.cancelOrder("b1", "u");
    expect(removed?.id).toBe("b1");
    expect(book.bids).toHaveLength(0);
  });

  test("cancelOrder returns null for an order that is not resting", () => {
    const book = new Orderbook([order({ id: "b1", userId: "u", side: "LONG", entryPrice: "99", quantity: "5" })], []);

    expect(book.cancelOrder("b1", "someone-else")).toBeNull();
    expect(book.cancelOrder("nope", "u")).toBeNull();
    expect(book.bids).toHaveLength(1);
  });

  test("cancelOrder returns the order with its authoritative filled size", () => {
    const book = new Orderbook([], []);
    book.addOrder(order({ id: "b1", userId: "u", side: "LONG", entryPrice: "100", quantity: "10" }));
    book.addOrder(order({ id: "a1", userId: "mk", side: "SHORT", entryPrice: "100", quantity: "4" }));

    const removed = book.cancelOrder("b1", "u");
    expect(removed?.filled).toBe("4");
  });
});
