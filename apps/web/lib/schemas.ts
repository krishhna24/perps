...
const isValidDecimalString = (v: string): boolean => {
  try {
    void new Decimal(v);
    return true;
  } catch {
...
export const positiveDecimalString = z
  ...
  .refine((v) => new Decimal(v).decimalPlaces() <= MONEY_DECIMAL_PLACES, {
...
export const depositSchema = z.object({ amount: positiveDecimalString });
